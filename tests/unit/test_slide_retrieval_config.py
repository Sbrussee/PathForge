...
def test_slide_retrieval_config_does_not_require_mil(tmp_path: Path) -> None:
    ...
    assert cfg.slide_retrieval.weights_dir == "model_weights/slide_retrieval"


def test_slide_retrieval_config_accepts_weights_directory(tmp_path: Path) -> None:
    annotation_path = tmp_path / "annotations.csv"
    annotation_path.write_text(
        "dataset,slide,category\ntrain,S1,tumor\n", encoding="utf-8"
    )
    ...
    cfg = Config.model_validate(
        {
            ...
            "slide_retrieval": {"weights_dir": "/shared/weights/slide-retrieval"},
            "datasets": [
                {
                    "name": "train",
                    "slides_dir": str(tmp_path),
    ...
    assert cfg.slide_retrieval is not None
    assert cfg.slide_retrieval.weights_dir == "/shared/weights/slide-retrieval"


def test_slide_retrieval_config_rejects_mil_dataset_use_semantics(
    tmp_path: Path,
) -> None:
    annotation_path = tmp_path / "annotations.csv"
    annotation_path.write_text(
...
def test_slide_retrieval_config_accepts_strategy_hyperparams(tmp_path: Path) -> None:
    ...
    assert cfg.benchmark_parameters.get_entries("retrieval_representation")[
        1
    ].hyperparams == {"percentile_threshold": 25}
