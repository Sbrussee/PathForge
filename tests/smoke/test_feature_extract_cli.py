...
"""Smoke tests for the single-slide feature-extraction CLI entry point."""
...
@pytest.mark.smoke
def test_feature_extract_slide_cli_importable():
    """The single-slide CLI module must be importable without side-effects."""
    from pathbench.cli import feature_extraction_slide  # noqa: F401


@pytest.mark.smoke
def test_feature_extract_slide_cli_missing_config(tmp_path):
...
@pytest.mark.smoke
def test_feature_extract_slide_cli_missing_slide(tmp_path):
    """main() with an existing config but nonexistent slide must raise FileNotFoundError."""
    from pathbench.cli.feature_extraction_slide import main
...
@pytest.mark.smoke
def test_feature_extraction_config_validates(minimal_fe_config):
    """A minimal feature-extraction config dict round-trips through Config validation."""
