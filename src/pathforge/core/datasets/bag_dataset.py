...
class BagDataset(BagDatasetBase):
    """Canonical H5 artifact-backed bag dataset."""
    ...
    def _init_artifact_mode(self, **kwargs: Any) -> None:
        ...
        if self.ds_cfg is None or self.annotations_df is None or self.combo_cfg is None:
            raise ValueError(
                "Artifact-backed BagDataset requires ds_cfg, annotations_df, and combo_cfg."
            )
    ...
    def _target_from_index(self, index: int) -> Any:
        ...
        if self.task in {"survival", "survival_discrete"}:
            ...
            target = {
                "time": torch.tensor(
                    int(metadata[time_column])
                    if is_discrete
                    else float(metadata[time_column]),
                    dtype=torch.long if is_discrete else torch.float32,
                ),
                "event": torch.tensor(
                    float(metadata[event_column]), dtype=torch.float32
                ),
            }
    ...
    def _annotations_frame(self) -> pd.DataFrame:
        if self.annotations_df is None:
            raise RuntimeError("Artifact-backed bag dataset has no annotations frame.")
    ...
    def _build_slide_samples(self, df: pd.DataFrame) -> list[BagSample]:
        ...
        slide_column = self._resolve_slide_column()
        for _, row in df.iterrows():
            slide_id = str(row[slide_column])
            ...
            samples.append(
                BagSample(
                    ...
                    patient_id=self._resolve_single_value(
                        row_df, PATIENT_ID_COL, cast=str
                    ),
    ...
    def _build_grouped_samples(
        self, df: pd.DataFrame, group_col: str
    ) -> list[BagSample]:
        samples: list[BagSample] = []
        for group_value, group_df in df.groupby(group_col, sort=False):
            slide_column = self._resolve_slide_column()
            group_df = group_df.sort_values(slide_column)
            slide_ids = [str(x) for x in group_df[slide_column].tolist()]
            ...
            samples.append(
                BagSample(
                    ...
                    patient_id=self._resolve_single_value(
                        group_df, PATIENT_ID_COL, cast=str
                    ),
    ...
    def _build_metadata(self, group_df: pd.DataFrame) -> dict[str, Any]:
        ...
        if self.task == "survival_discrete":
            tracked_columns.add(
                self._resolve_continuous_time_column(self._resolve_time_column())
            )
        tracked_columns.discard(None)
        for column in group_df.columns:
            if column not in tracked_columns and column not in {
                DATASET_COL,
                SLIDE_ID_COL,
            }:
                continue
    ...
    def _ensure_features_exist(
        ...
        *,
        ...
        if self.tiling_id is None or self.extractor_name is None:
            raise RuntimeError(
                "Artifact-backed feature checks require tiling_id and extractor_name."
            )
    ...
    def _load_slide_bag(self, artifact_path: Path) -> torch.Tensor:
        if self.tiling_id is None or self.extractor_name is None:
            raise RuntimeError(
                "Artifact-backed bag loading requires tiling_id and extractor_name."
            )
        with FileHandleH5(artifact_path, mode="r") as slide_artifact:
            feature_matrix = features_io.read_features(
                slide_artifact,
