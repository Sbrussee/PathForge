# src/pathforge/core/io/h5/tissue.py
...
def tissue_exists(slide_artifact: FileHandleH5, *, layout: H5Layout = DEFAULT_LAYOUT) -> bool:
    ...
    if dset is None or not is_complete(dset):
        return False
    try:
        _ensure_tissue_polygons(read_json_dataset(slide_artifact.h5, layout.tissue_dataset))
    except Exception:
        return False
    return True
...
def read_tissue(
    ...
    *,
    ...
    dset = get_dataset(slide_artifact.h5, layout.tissue_dataset)
    if dset is None:
        raise KeyError(f"Missing tissue dataset: {layout.tissue_dataset}")
    if not is_complete(dset):
        raise ValueError(f"Tissue dataset is incomplete: {layout.tissue_dataset}")
