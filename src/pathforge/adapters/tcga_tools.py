...
def resolve_external_dataset_sources(
    ...
    *,
    ...
    """Resolve TCGA/TCIA dataset declarations into local PathForge datasets.

    Parameters
    ----------
    config_data:
        Mutable YAML payload parsed from a PathForge config file.
    config_path:
    ...
    datasets_root = _pathforge_repo_root() / "datasets"
    ...
    annotation_output = datasets_root / "pathforge_external_annotations.csv"
...
def _load_tcga_tools_module():
    if importlib.util.find_spec("tcga_tools") is None:
        tcga_repo = _pathforge_repo_root().parent / "TCGA_TOOLS_2.0"
        repo_str = str(tcga_repo)
        if not tcga_repo.is_dir():
            raise ImportError(
                "tcga-tools is required for external dataset integration. "
                "Install it or add ../TCGA_TOOLS_2.0 next to PathForge."
            )
...
def _concrete_dataset_entry(
    *,
    ...
    return {
        ...
        "artifacts_dir": str((dataset_dir / "pathforge_artifacts" / role).resolve()),
