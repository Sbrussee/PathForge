...
All classes inherit from :class:`~pathbench.core.models.base.ScikitBase`
(which in turn inherits from :class:`~pathbench.core.models.base.ModelBase`)
so they participate in the shared PathBench model hierarchy without
...
They are trained via :class:`~pathbench.training.sklearn_trainer.SklearnSlideTrainer`
rather than :class:`~pathbench.training.lightning.LightningTrainer`.
...
:data:`SKLEARN_ESTIMATOR_CATALOG` maps PathBench model names to
...
# ---------------------------------------------------------------------------
# Catalog: PathBench name → (sklearn_module, class_name, task, fixed_kwargs)
# ---------------------------------------------------------------------------
...
def list_sklearn_slide_models(task: Optional[str] = None) -> list[str]:
    ...
    Returns:
        Sorted list of PathBench model names whose underlying sklearn/sksurv
...
def make_sklearn_slide_model(
    ...
    Returns:
        A fitted-ready :class:`~pathbench.core.models.base.ScikitBase` instance.
