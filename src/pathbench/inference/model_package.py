...
class LoadedModelPackage:
    """Self-contained packaged PathBench MIL model.
    ...
    @property
    def task(self) -> str:
        """Return the PathBench task name stored in the package metadata."""
...
def save_packaged_model(
    *,
    ...
    Args:
        ...
        config: Active PathBench config used to reconstruct the architecture.
    ...
    payload = {
        "format": "pathbench_model_package",
        "format_version": MODEL_PACKAGE_FORMAT_VERSION,
        "pathbench_version": _pathbench_version(),
...
def load_packaged_model(
    ...
    *,
    ...
    """Load one packaged PathBench MIL model from disk.
...
def predict_bag(
    ...
    *,
    ...
    Args:
        ...
        task: PathBench task name controlling output normalization.
...
def _validate_model_package_payload(payload: Any, path: Path) -> None:
    ...
    if payload.get("format") != "pathbench_model_package":
        raise ValueError(f"{path} is not a PathBench packaged model artifact.")
    required_keys = {
        "config",
        "format_version",
        "input_dim",
